<template>
                ...
                <form @submit.prevent="handleSubmit" class="w-full p-6 text-lg">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 ">
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <select id="esdType" v-model="esd.esdType"
                        ...
                        <div class="flex flex-col w-full col-span-1 space-y-2 md:col-span-2">
                            ...
                            <input id="remark" v-model="esd.remark"
                                class="w-full input bg-white text-black border border-black text-[20px] focus:outline-none"
    ...
    <div class="flex items-center justify-center p-4">
        <div class="w-[100%] max-w-7xl shadow-sm card bg-white mt-5 p-2">
            <h2 class="mt-5 text-2xl font-semibold text-center text-black">ตารางผู้ที่ได้รับการยกเว้น Check ESD
                แต่ละประเภท</h2>
            <div class="flex items-center justify-end mt-4">
                <input type="text" placeholder="ค้นหารหัสพนักงาน"
                    class="input bg-white text-black border border-black text-[16px] focus:outline-none"
                    @input="SearchFilterESD" v-model="esd_empid" />
                <select class="input bg-white text-black border border-black text-[16px] focus:outline-none ms-2"
                    @change="SearchFilterESD" v-model="esd_section">
                    <option value="" disabled selected>--เลือกแผนกที่ต้องการ--</option>
                    <option v-for="(sec,index) in esd_sec" :key="index" :value="sec.VEMPLOYEE_SECTION">
                ...
                <table class="table w-full min-w-[2000px] ">
                    ...
                    <tbody>
                        <tr v-for="esd_exc, index in esd_db" :key="index" class="text-center">
                            ...
                            <td class="text-[18px] font-semibold text-gray-900">{{ esd_exc.VEMPLOYEE_THPREFIX }}{{
                                esd_exc.VEMPLOYEE_THFNAME }}&nbsp;{{
                                    esd_exc.VEMPLOYEE_THLNAME }}</td>
                            <td class="text-[18px] font-semibold">
                                <div class="badge badge-outline badge-warning p-5">{{ esd_exc.TExcludeEsd_EsdTy }}</div>
                            </td>
                            <td class="text-[18px] font-semibold text-gray-900">{{ esd_exc.VEMPLOYEE_SECTION }}</td>
...
const fetch_db_sec = () =>{
    axios.get('/CheckESD/api/data-sec')
    .then(res =>{
